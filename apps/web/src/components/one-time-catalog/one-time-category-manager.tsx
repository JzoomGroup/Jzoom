...
export function OneTimeCategoryManager({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const saved = await mutation.mutate(
      ...
      creating ? "One-time category created." : "One-time category updated.",
    );
    if (saved) {
      closeForm();
    }
  ...
  return (
    <>
      <SectionHeader
        ...
        title="One-time service categories"
      ...
      {(creating || editing) && (
        <section className="catalog-panel editor-panel">
          <div className="panel-heading">
            <div>
              <h2>{creating ? "New one-time category" : `Edit ${editing!.code}`}</h2>
      ...
      <section className="catalog-panel">
        ...
        {snapshot.categories.length === 0 ? (
          <EmptyState>No one-time categories exist yet.</EmptyState>
        ) : (
